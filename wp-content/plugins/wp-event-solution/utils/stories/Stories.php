...
namespace Eventin\Utils\Stories;
...
class Stories
{
    ...
    private function get_stories()
    {
        ...
        if ($this->check_interval + $this->last_check < time()) {
            $response = wp_remote_get($this->api_url . 'cache/stories.json', array('timeout' => 10, 'httpversion' => '1.1'));

            if (!is_wp_error($response) && isset($response['body']) && $response['body'] != '') {
                $response = json_decode($response['body']);
                if (!empty($response)) {
                    $this->data = map_deep( (array) $response, 'sanitize_text_field' );
    ...
    public function show_story_widget()
    {
        ...
        wp_add_dashboard_widget('wpmet-stories', $this->title ?? esc_html__('Wpmet Stories', 'eventin'), array($this, 'show'), null, null, 'normal', 'high');
